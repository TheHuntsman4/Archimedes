  ...
  const handleSubmit = () => {
    ...
    axios
      ...
      .then((res) => {
        ...
        setResponse(res.data.generatedOutput); // Extract the generated output from the response
      })
      .catch((error) => {
        console.log(error);
      });
  ...
  return (
    <>
      <div className="grid md:grid-cols-2 grid-cols-1 justify-center align-middle rounded-lg bg-gradient-to-b from-[#1269af] to-transparent p-[2rem] ease-in-out duration-300 hover:scale-[1.15] shadow-2xl shadow-black" onClick={toggleModal}>
        <img className="h-[10rem] w-[50%] md:w-full justify-self-center place-items-center" src={image} alt={title} />
        <div className="flex flex-col justify-start align-middle">
          <p className="text-[1rem] md:text-[2vh] lg:text-[1.25vw] font-Archivo font-bold text-left text-[#DEB992] px-[1.5rem] ">{title.length > 20 ? title.substring(0, 20) + "..." : title}</p>
        ...
        <div className="modal">
          ...
          <div className="absolute top-1/2 left-1/2 bg-[#0C1039] text-[#DEB992] -translate-x-1/2 -translate-y-1/2  h-[50%] w-auto overflow-y-auto overflow-x-hidden p-[2rem] rounded-lg ">
            <div className="grid lg:grid-cols-2 gap-2 p-[1rem] sm:grid-cols-1">
              <div><img src={image} className=" h-[17rem] w-[14rem] lg:h-full lg:w-full pr-[1rem] rounded-lg" alt={title} /></div>
              <div className="text-left uppercase font-bold text-[2rem] font-Roboto">{title}
                ...
                <div className="text-left uppercase text-[1rem] pb-[1rem] font-Archivo"><a target="_blank" rel="noopener noreferrer" href={preview}>Live Preview Link</a></div>
                <div className="text-left text-[1rem] font-Archivo">{description ? description : "Description unavailable"}</div>
              </div>
            </div>
            <button className="text-[1.5rem] bg-[#0C1039] rounded-full p-[1vh] hover:bg-[#DEB992] hover:text-[#0C1039]" onClick={handleSubmit}>
              Generate
            </button>
            ...
            <button className="close-modal" onClick={toggleModal}>
              <Cross className="translate-x-1/2 h-[20px]" alt="Close Modal" />
            </button>
          </div>
        </div>
      )}
